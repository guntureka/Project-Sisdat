...
if($op == 'delete'){
    $id         = $_GET['id'];
    $q1         = mysqli_query($koneksi,"delete from muzakki where id_muzakki = '$id'");
    if($q1){
...
if ($op == 'edit') {
    $id         = $_GET['id'];
    $sql1       = "select * from muzakki where id_muzakki = '$id'";
...
if(isset($_POST['daftar'])){
    ...
    if($nama && $nik && $kelamin && $alamat){
        if($op == 'edit'){
            $cek = mysqli_query($koneksi, "update muzakki set id_muzakki = '$id', nama = '$nama', nik = '$nik', kelamin = '$kelamin', alamat = '$alamat' where id_muzakki = '$id'");
            if($cek == 1){
            ...
            $cek = mysqli_query($koneksi, "INSERT INTO muzakki(nama,nik,kelamin,alamat) VALUES ('$nama','$nik','$kelamin','$alamat')");
            if($cek == 1){
                $sukses = "Data berhasil diinput";
            }else{
                $error = "Data tidak berhasil diinput";
...
<body>
    ...
    <section>
        ...
        <?php
            header("refresh:3;url=muzakki.php");
        }
        ?>
        <?php
        if($sukses){
        ...
        ?>
          <div class="muzakki border-bottom pb-3">
        ...
        <div class="container">
            <table class="table">
                ...
                <tbody>
                    <?php
                        $cek = mysqli_query($koneksi, "select * from muzakki order by id_muzakki asc");
                        $urut = 1;
                    ...
                    <tr>
                        ...
                        <td scope="row">
                            <a href="muzakki.php?op=edit&id=<?php echo $id ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                            <a href="muzakki.php?op=delete&id=<?php echo $id?>" onclick="return confirm('Yakin mau delete data?')"><button type="button" class="btn btn-danger">Delete</button></a>     
                        </td>
                    </tr>
                    <?php
                        }
