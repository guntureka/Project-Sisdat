...
if($op == 'delete'){
    $id         = $_GET['id'];
    $q1         = mysqli_query($koneksi,"delete from mustahiq where id_mustahiq = '$id'");
    if($q1){
...
if ($op == 'edit') {
    $id         = $_GET['id'];
    $sql1       = "select * from mustahiq where id_mustahiq = '$id'";
...
if(isset($_POST['daftar'])){
    ...
    if($nama && $nik && $kelamin && $alamat){
        if($op == 'edit'){
            $cek = mysqli_query($koneksi, "update mustahiq set id_mustahiq = '$id', nama = '$nama', nik = '$nik', kelamin = '$kelamin', alamat = '$alamat' where id_mustahiq = '$id'");
            if($cek == 1){
            ...
            $cek2 = mysqli_query($koneksi, "INSERT INTO mustahiq(nama,nik,kelamin,alamat) VALUES ('$nama','$nik','$kelamin','$alamat')");
            if($cek2 == 1){
                $sukses = "Data berhasil diinput";
            }else{
                $error = "Data tidak berhasil diinput";
...
<body>
    ...
    <section>
        ...
        <?php
            header("refresh:5;url=mustahiq.php");
        }
        ?>
        <?php
        if($sukses){
        ...
        ?>
          <div class="mustahiq border-bottom pb-3">
        ...
        <div class="container">
            <table class="table">
                ...
                <tbody>
                    <?php
                        $cek = mysqli_query($koneksi, "select * from mustahiq order by id_mustahiq desc");
                        $urut = 1;
                    ...
                    <tr>
                        ...
                        <td scope="row">
                            <a href="mustahiq.php?op=edit&id=<?php echo $id ?>"><button type="button" class="btn btn-warning">Edit</button></a>
                            <a href="mustahiq.php?op=delete&id=<?php echo $id?>" onclick="return confirm('Yakin mau delete data?')"><button type="button" class="btn btn-danger">Delete</button></a>     
                        </td>
                    </tr>
                    <?php
                        }
